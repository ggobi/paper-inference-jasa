...
Notice that now we can fix $type-I$ error to some values even less than $1/m$ by increasing $K$. $type-I$ error is $1/m$ only when $K=1$.
}
...
\green{check that we consistently use $1/m$, or $\frac{1}{m}$ in the text.} \blue{its jasa guideline to use fraction like $1/m$ whenever possible to save space}.
