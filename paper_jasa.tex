...
\begin{figure}[hbtp]
   \centering
       \scalebox{0.70}{\includegraphics{power_loess_effect.pdf}}
...
In all the three experiments, each subject saw a lineup only once. Assuming that there is no random effect for lineup, we set  $\tau_{\ell  i}=\tau_i $ in model \ref{mixed} and $W_{\ell i} = E_{\ell i}$, the effect of lineup $\ell$ that subject $i$ has evaluated. This gives us the estimate of subject specific probability of successful evaluation of a lineup ($\hat p_i$) from model \ref{mixed}. Parameter estimates of the model is shown in table \ref{tbl:model_par}.  The estimated subject specific power curves are shown in figure \ref{fig:power_mixed_subject}. The offset is set to $log(0.05/0.95)$ so that Type-I error is fixed to be 0.05 when effect = 0. Subject specific power is quite different for three experiments. It appears that scatter plot serves better job compared to boxplot as a visual test statistic. But subject to subject variability is larger in scatterplot as we see in figure \ref{fig:power_mixed_subject} for experiment 2 and 3.
...
\begin{figure}[hbtp]
   \centering
       ...
       \caption{Subject specific  power estimated from model \ref{mixed} are shown. The corresponding power curve for conventional test is shown for comparison.}
