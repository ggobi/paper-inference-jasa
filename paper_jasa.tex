...
%\]
This implies that as $p_D$, the $p$-value, becomes smaller, the probability that the observer identifies the data plot goes up and \blue{(How can we say that from this? The term  $E\left[ (1 - p_D)^{m-1}\right]$ is definitely not a function of $p_D$ since $p_D$ should be eliminated after the expectation. Instead, its a function of $m$ only ! Is this interesting that under the minimum $p$-value assumption the power is a function of lineup size $m$ only?)},
...
\begin{figure*}[hbtp]
   \centering
       \scalebox{0.55}{\includegraphics{p_val_definition.pdf}}
       \caption{Conventional test $p$-value ($p_D$) vs visual $p$-value obtained from the definition . Values are shown on square root scale scale. For experiment 3, $p$-values of uncontaminated part of the data are also shown.}
