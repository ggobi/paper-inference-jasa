...
Similary, the strength of the signal, is calculated using the proportion of observers picking actual data plot. It is closely related to the power of the test. From Equation \eqref{eqn:power_estimate}  we can write $1 - E[N_m]/K \approx (m-1)E[p_D] $. This gives the signal strength estimate as  
\[
\hat p_D= (1- {u}/{K})/({m-1})
\]
...
\begin{figure*}[hbtp]
   \centering
       ...
       \caption{Conventional test $p$-value ($p_D$) vs plot signal strength ($\hat p_D$). The solid line is a 45 degree line which represents the equity shown in Equation \eqref{eqn:power_estimate}. p-values are shown on square root scale scale. \blue{Is it better now?}}
    %   \hh{Mahbub, another change: could you change the y label to 'estimate of visual p-value $\hat{p_D}$ ?}
