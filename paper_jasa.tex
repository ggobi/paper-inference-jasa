...
\begin {abstract}  
Statistical graphics play a crucial role in exploratory data analysis, model checking and diagnosis. A new method, called the lineup protocol, \red{enables determining} the statistical significance of visual findings, which bridges a big gulf between exploratory and inferential statistics.  In this paper inferential methods for statistical graphics are developed further by refining the terminology of visual inference, and framing the lineup protocol in a context that allows direct comparison with conventional tests, in scenarios when a conventional test exists. This framework is used to compare the performance of the lineup protocol against conventional statistical testing in the scenario of fitting linear models. A human subjects experiment is conducted using simulated data to provide controlled conditions. \blue{Results suggest that the lineup protocol provides comparable results to the conventional tests, and expectedly out-performs them when signal is statistically significant or data is contaminated, a scenario where assumptions required for performing a conventional test are violated, and there may be some super-individuals who yield better power than the conventional test even in the most difficult tasks.}
...
Note that $\tau_{\ell i }$ usually only includes a partial interaction; for a full interaction of subjects' skills and lineup-specific difficulty we would need replicates of the same subject evaluating the same lineup, which in practice is not feasible without \red{loosing} independence.
...
\end{equation}
Figure \ref{fig:power_expected} shows the power of the conventional test in comparison to the expected power of the visual test for different number of observers ($K$) obtained using $p(\beta)$ from Equation \ref{power_exp}. Notice that the expected power of the visual test exceeds the power of the conventional test as $K$ gets larger.  }
...
\begin{figure}[hbtp]
   \centering
       ...
       \caption{Expected power of visual test  for different number of observers ($K$) and the power of conventional test for sample size $n =100$, $\sigma = 12$ and $m$=20. }
...
Figure \ref{fig:test_continuous} shows an example lineup for this experiment from difficulty level 4.  This lineup is generated using sample size ($n$)=100, slope($\beta$)=1.25 and and error SD($\sigma$)=5. \blue{The actual data plot location is the integer value nearest to $log_e(148.4132)$. None of the 65 observers picked the actual plot while 46 observers picked plot 18 which has the lowest $p$-value among all the plots in this lineup as we can see in Figure \ref{fig:P-val_log2} for Experiment 2.}
...
Columns are ordered according to effect size, with rows showing replicates of the same parameter combination on top of each other. Empty cells indicate combination of parameters with fewer than five replicates. Highest counts tend to be the plot in the lineup having the lowest $p$-value, even more so than for experiment 1.
    }   }
