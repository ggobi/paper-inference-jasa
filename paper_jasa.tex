...
\[
P(p_D < p_0) = 1 - E\left[ (1 - p_D)^{m-1}\right]
\]
...
\[
F_{p_0} (x) =  (1- x) ^{m-1} \text{ for } x \in [0,1].
\]

\begin{eqnarray*}
P(p_D < p_{0}) &=& 1 - P(p_{0} \le p_D) = 1- \int_0^1  P(p_{0} \le p_D \mid p_D=t) f_{p_D}(t) dt =  \\
&=& 1 - \int_0^1 F_{p_{0}}(t) f_{p_D}(t) dt = 1 - \int_0^1 (1-t)^{m-1} f_{p_D}(t) dt = 1 - E\left[ (1 - p_D)^{m-1}\right] = \\
&=& 1 - \sum_{k=0}^{m-1} {m-1 \choose k} (-E[p_D])^{k} = (m-1)E[p_D] + O(E[p_D^2]),
\end{eqnarray*}

This can't be right - it's completely counter-intuitive: as $p_D$, the $p$-value is smaller, the probability that we pick the data plot should go up not down. 
Conversely, as $m$, the number of choices given, goes up the probability to pick the data plot should go down, not up. 
}
%
%
%
...
N_m\sim \left \{ 
\begin{array}{ll}
B_{K, 1-E[1 - p_D])^{m-1}} & \text { under } H_1\\
...
\begin{figure}[hbtp]
   \centering
       \scalebox{.95}{\includegraphics{power_expected.pdf}}
...
{\bf Cleaning data:} There are six suggested approach to clean the data. They are 
\begin{enumerate}
...
\item each person saw at least one easy lineup. we select one of those easy lineups as the inclusion criteria. If that lineup is evaluated correctly we include only the rest of the evaluations of that subject. Otherwise, we exclude all the feedbacks of that subject.
\end{enumerate}
...
\begin{figure*}[hbtp]
   \centering
       \scalebox{0.6}{\includegraphics{p_val_rank_counts.pdf}}
       \caption{Rank of p-values in a lineup of size 20 and the number of subjects choosing that plot in the lineup of experiment 1 with raw data (nothing is dropped). This indicates that most of the subjects pick the plot with minimum p-value (rank 1). There are subjects choosing the lineups with highest p-value(rank 20) but we can attribute this behavior to the type-I error. Can we?}
