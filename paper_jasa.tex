...
Statistical graphics plays a vital role in discovery. Finding the unexpected in data, structure that was not previously anticipated or covered by the original hypotheses is one of the strengths of visual analysis. This was an area pioneered by \cite{tukey:eda} and is particularly pertinent in today's data-rich world where data mining has become an important activity. Graphics are also used in many places where numerical summaries simply do not suffice: model checking, diagnosis, and in the communication of findings. 

The foundation of statistical graphics is well-established. Work by \cite{cleveland:1984}, repeated and expanded by \cite{heer:2010} provides a framework for evaluating human perception of statistical graphics.  \cite{wilkinson:1999} has developed a grammar of graphics which has been enhanced by \cite{hadley:2009} that formalizes statistics graphics helping to describes how different plots are related. The availability of software systems enabling beautiful data graphics to be tightly coupled with modeling, such as R \citep{R}.

But the strength of the patterns revealed by plots or individual differences among the audience members is something difficult to quantify. \citet{buja:2009}, following from \cite{gelman:2004}, proposed two protocols that allow the testing of discoveries made from statistical graphics. This work represents a major advance for graphics, because it bridges the gulf between classical statistical inference procedures and exploratory data analysis.
