...
\begin{itemize}\itemsep-3pt
\item \textbf{Reject} $H_0$ if out of $K$ observers $x_{\alpha}$ of them correctly identify the observed data plot in the lineup, and
\item \textbf{Fail to reject} $H_0$  otherwise. %if the observer selects a null plot.
\end{itemize}
...
% identifying the observed data plot, when $H_0$ is true, which is $1/m$, and is thus controlled by the number of null plots. 
\textbf{Type II error} is the probability of failing to identify the observed data plot, when $H_0$ is not true, $P( X <  x_{\alpha})$, where $x_{\alpha}$ is the (approximate) $\alpha$ quantile of  $B_{K, 1/m}$. \green{Is this Binom????}
\end{dfn}

\noindent Because $X$ takes only discrete values we can not always control exactly for $\alpha$, finding an exact $x_{\alpha}$. For example, when there is only one observer, $1/m$ is the minimal value at which we can set $\alpha$. It can be set to be smaller, even arbitrarily small, by increasing $K$. 

\green{check that we consistently use $1/m$, or $\frac{1}{m}$ in the text.} \blue{its jasa guideline to use fraction like $1/m$ whenever possible to save space}. \green{so please make sure that they are all $1/m$.}
 
%The number of plots in the lineup, $m$, controls the Type I error. The probability of choosing any one of the plots is $1/m$, so the probability of making a wrong decision by chance is $1/m$. 


\green{What does this part apply to ???? power?}let $X_i$ be a binary random variable with $X_i = 1$, if individual $i$ identifies the data plot from the lineup, and $X_i = 0$ otherwise, \red{where $p_i$ is the probability that individual $i$ picks out the data plot from the lineup. 
...
The \textbf{power} of a visual test $V_{\theta}$ is defined as the probability to reject the null hypothesis for a given parameter value $\theta$:
    \begin{equation*}
      \text{Power}_V(\theta)= Pr(\text{Reject } H_0) = P(X \ge x_{1-\alpha}) = 
        \begin{cases} 
               1 - \text{Binom}_{K, 1/m}(x_{1-\alpha} - 1) & \text{ if } H_0 \text{ is true }.\\
              1 - F_{X, \theta} (x_{1-\alpha} - 1) & \text{ if } H_1 \text{ is true}.
        \end{cases}
    \end{equation*}
\end{dfn}

\noindent
\red{Note that $F_{X, \theta}$ is the distribution of $X$ under the alternative with given parameter value $\theta$.}
...
In the case that observers are exposed to multiple lineups we can estimate the probability $p_i$ while accommodating both for different abilities of individuals as well as differences in the difficulty of lineups with the help of a logistic regression using random effects: 


Let $X_{j\ell} \sim B_{1, \pi_{j\ell}}$ be the random variable, with $X_{j\ell}= 1$, if  subject $j$ correctly identifies the data plot from lineup $\ell$, and 0 otherwise.
...
Then $Z \sim B_{m-1, E[p_D]}$, and the probability that an onlooker will pick the data plot in a given lineup is 
\[
...
\begin{figure*}[hbtp]
   \centering
       \scalebox{0.7}{\includegraphics{effect_power_exp1.pdf}}
       \caption{Observed power shown by loess smoother and the power of UMP test for different effect size $E$ as per experiment 1.}
       \label{fig:power_effect1}
\end{figure*}

\begin{figure*}[hbtp]
   \centering
       \scalebox{0.7}{\includegraphics{effect_power_subject_exp1.pdf}}
       \caption{Subject specific power obtained from equation \ref{eqn:effect} and the power of UMP test for different effect size $E$ as per experiment 1.}
