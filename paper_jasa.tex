...
\begin{table*}[hbtp]
\caption{Comparison of visual inference with existing inference technique. *** The plot for the traditional test is wrong, for two sided, p-value is area to the right and left of $|T(y|$. (MM: the vertical bar is showing the observed $T$ location not p-value.) }
\centering 
\begin{tabular}{llll} 
\hline
...
In general, let $\theta$ be a population parameter of interest, with $\theta \in \Theta$, the parameter space. Any null hypothesis $H_0$ then partitions the parameter space into $\Theta_0$ and $\Theta_0^c$, with $H_0: \theta \in \Theta_0$ versus $H_1: \theta \in \Theta_0^c$. Similarly, for any test, we have a rejection region $R$ and, complementary to that, $R^c$,  that partition the space of the test statistic, which is usually a subspace of the real numbers. For visual inference, the statistic is -- unlike in the classical hypothesis test -- not a single value, but a graphical representation of the data chosen to display the strength of the parameter of interest, $\theta$. When the alternative hypothesis is true, it is expected that the plot of the observed data, the test statistic, will have visible feature(s) consistent with $\theta \in \Theta_0^c$, and that visual artifacts will not distinguish the test statistic as different when $H_1$ is not true. We will call a plot with this property a {\it visual statistic} for $\theta$. More formally, 

\begin{dfn} \label{dfn:test}
A \textbf{visual test (statistic)} $T_\theta$ is defined as the grammar \citep{wilkinson:1999} of the plot,  consisting of type and specification of aesthetics necessary for complete reproducibility.
\end{dfn}

The visual test statistic is compared with values $ \theta \in \Theta_0$ using a lineup, which is defined as:

\begin{dfn}\label{dfn:lplot}
A \textbf{lineup}  is a layout of $m$ visual statistics, consisting of 
\begin{itemize}\itemsep-3pt
\item $m-1$ statistics simulated from the model specified by $H_0$  (null plots) and 
