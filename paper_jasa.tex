...
\hh{If there is evidence for individual skills influencing the probability $p_i$, then $X_i \sim Binom_{1, p+i}$ and $X$ is a sum of independent Bernoulli random variables with different success rates $p_i$. This makes the distribution of $X$ a Poisson-Binomial by definition, see XXX for details. estimating $p_i$ will be one of the main concerns for the remainder of the paper.}
...
\[
\text{Power}_V(\theta)= 1 - Binom_{K, 11/m} (x_\alpha - 1),
\]
...
g( p_{\ell i} )= X_{\ell i} B +  Z_{\ell i}  \tau_{i\ell},  
\end{equation}
...
$Z_{i\ell}$ & $1 \le i \le K$, $1 \le \ell \le L$  is a design matrix corresponding to random effects specific to individual $i$ and lineup $\ell$.  \\
$\tau$ & is a vector of independent real-valued random variables $\tau_{i\ell}$,  $\tau  \sim  MVN(0,\sigma_\tau I_{KL \times KL})$. $\tau$ will usually include a component incorporating an individual's ability or skill to evaluate lineups.\\
\end{tabular}
...
In this setting, there is a multitude of established graphics that help with the evaluation and diagnostics of a model \citep{cook:99}. Table \ref{tbl:stat_multiple} shows an overview of possible hypotheses that would be tested in the regression setting, and what visual test statistic might be used in these tests. These plots shoul be familiar, because they are all commonly used for evaluating and diagnosing regression models. For example, to examine the effect of variable $X_j$ on $Y$, we would plot $Y$ against $X_j$ (cases 1-4). To assess whether the linear assumption is appropriate we would make a plot of residuals against fitted values (case 5). 
...
\hline 
1 & $H_0: \beta_0=0$ & Scatter plot & \begin{minipage}[t]{3cm}\begin{center}  	\scalebox{0.1}{\includegraphics{stat_intercept.pdf}} \end{center} \end{minipage} & Scatter plot with least square line overlaid. For lineup plot, we simulate data from fitted null model. \\ 
...
3 & $H_0: \beta_k=0$ (for categorical $X_k$) & Box plot & \begin{minipage}[t]{3cm}\begin{center}  	\scalebox{0.4}{\includegraphics{stat_category.pdf}} \end{center} \end{minipage} & Box plot of residuals grouped by category of $X_k$. For lineup plot, we simulate data from normal with mean 0 variance $\hat{\sigma}^2$. \\
4 & $H_0: \beta_k=0$ (interaction with categorical $X_k$) & Scatter plot & \begin{minipage}[t]{3cm}\begin{center}  \scalebox{0.11}{\includegraphics{stat_interection.pdf}} \end{center} \end{minipage} & Scatter plot with least square lines of each category overlaid. For lineup plot, we simulate data from fitted null model.\\[1ex] % [1ex] adds vertical space 
5 & $H_0: X$ Linear & Residual Plot & \begin{minipage}[t]{3cm}\begin{center}  	\scalebox{0.1}{\includegraphics{stat_nonlinear.pdf}} \end{center} \end{minipage} & Residual vs predictor plots with loess smoother overlaid. For lineup plot, we simulate residual data from normal with mean 0 variance $\hat{\sigma}^2$. \\ 
6 & $H_0: \sigma^2=\sigma^2_0$ & Box plot & \begin{minipage}[t]{3cm}\begin{center}  \scalebox{0.15}{\includegraphics{stat_sigma_box.pdf}} \end{center} \end{minipage} & Box plot of standardized residual divided by  $\sigma^2_0$. For lineup plot, we simulate data from standard normal. \\ 				
7 & $H_0: \rho_{X,Y|Z}=\rho$ & Scatter Plot & \begin{minipage}[t]{3cm}\begin{center}  	\scalebox{0.1}{\includegraphics{stat_intercept.pdf}} \end{center} \end{minipage} & Scatter plot of Residuals obtained by fitting partial regression. For lineup plot, we simulate data (mean 0 and variance 1) with specific correlation $\rho$. \\ 
8 & $H_0:$ Model Fits & Histogram & \begin{minipage}[t]{3cm}\begin{center}  \scalebox{0.1}{\includegraphics{stat_goodness_simple.pdf}} \end{center} \end{minipage} & Histogram of the response data. For lineup plot, we simulate data from fitted model. \\[1ex] % [1ex] adds vertical space 
9 & Special case $p=1$ $H_0: \rho_{X,Y} =\rho$ & Scatter plot & \begin{minipage}[t]{3cm}\begin{center}  \scalebox{0.1}{\includegraphics{stat_intercept.pdf}} \end{center} \end{minipage} & Scatter plot with least square line overlaid For lineup plot, we simulate data with correlation $\rho$.\\			
\hline 
\end{tabular} 
\label{tbl:stat_multiple} 
\end{table*} 

For the purposes of validating visual inference, we focus on cases 2 and 3, with a continuous and catgorical explanatory variable respectively. 
...
The classical uniformly most powerful (UMP) test rejects $H_0$ if the observed test statistic $T= \hat{\beta_k}/ se(\hat{\beta_k})$ takes an extreme value in the distribution of theoretical $t$ distribution with $n-p$ degrees of freedom. Figure \ref{fig:power_expected} shows the power of the UMP test in comparison to the expected power of the visual test obtained from equation \eqref{power_exp}. Notice that the expected power of the visual test is almost as good as the power of UMP test. In fact, the only loss we experience stems from comparing the test statistic to only a discrete number of  $m-1$ samples from the null distribution. 
...
%\end{enumerate}
 ...
 %Figure \ref{fig:test_category} is an example of one of these lineups. The visual test statistic is a side-by-side boxplot. The actual data plot is number 13 which was generated for $n$=300, $\beta_2$=15 and $\sigma$=12.  
...
This experiment is designed for the same regression model (Equation \ref{multi}) with $p=1$ and covariate $X_1$ being continuous. $X_1$ is simulated from standard normal(0,1). Data is simulated with two sample sizes ($n=100, 300$) and two standard deviations of the error ($\sigma=5, 12$). The different values for slope parameter that are used to generate data are shown in Table \ref{tbl:experiment_params}. We arbitrarily fix $\beta_0 = 6$ while generating actual data. For each combination of parameter, we generate at least three different actual data sets constituting a total of 65 lineups. For some parameter combinations we have more than three replications. Unlike what we did in section \ref{sec:category} we did not use any blocking structure to simulate actual data and null plots.
...
\[
  ...
  \end{array} \right.
\]
where $\epsilon_i \stackrel{iid}\sim N(0,\sigma)$, $\eta_i \stackrel{iid}\sim N(0,\sigma/3)$ and $\mu = -1.75$, $c$ is the proportion of contaminated data. For the experiment we consider $n=100$, $c=0.15$ which produced actual data size 115, $\alpha=0$ and $\lambda=10$ . $\sigma$ is chosen such that overall error standard deviation of the data is 5 which is approximately 3.5. One such plot of the contaminated data for  $\beta$=5 is shown in figure \ref{fig:cont_dat}.  We fit model (Equation \eqref{multi}) with $p=1$  and intercept $\beta_0=0$ to this contaminated data to test the hypothesis whether slope is zero or not.
...
One lineup for slope $\beta=0.4$ is shown in figure \ref{fig:test_contaminated}. For this experiment we generate five replications of actual data sets for each slope value shown in table \ref{tbl:experiment_params}. This produces 30 lineups. each subject got at least 10 lineups to evaluate. Based on the slope parameter we group lineups into six groups with difficulty = 0 for largest slope and difficulty = 5 for the smallest slope. Eight (8) lineups with two from each of the difficulty group from 0 through 3 and two(2) lineups with one from each difficulty group 4 and 5 were randomly picked for each subject.  
...
{\bf Cleaning data:} There are five suggested approach to clean the data. They are 

\begin{enumerate}
\item not throw out anything
\item throw out people where we don't have demographic information
\item throw out all people with 0 percent correct
\item only include people with at least 20 percent correct (current suggestion)
\item include people with 50\% on all easy lineups (as suggested before)
\end{enumerate}

\begin{table}[hbtp]
\caption{Summary of unique subjects and their total feedbacks after applying five screening process on all three experimental data}\begin{center}\begin{tabular}{rrrrr|rrrr|rrrr}  \hline
Screening &  \multicolumn{4}{c} {Experiment 1}  & \multicolumn{4}{c} {Experiment 2}  & \multicolumn{4}{c} {Experiment 3} \\
 \cline{2-5}  \cline{6-9}   \cline{10-13}criteria & total & male & female & feedbacks & total & male & female & feedbacks & total & male & female & feedbacks \\   \hline1 & 520 & 264 & 201 & 4914 & 520 & 264 & 201 & 4914 & 520 & 264 & 201 & 4914 \\   2 & 465 & 264 & 201 & 4624 & 465 & 264 & 201 & 4624 & 465 & 264 & 201 & 4624 \\   3 & 384 & 200 & 165 & 4187 & 384 & 200 & 165 & 4187 & 384 & 200 & 165 & 4187 \\   4 & 318 & 168 & 135 & 3395 & 318 & 168 & 135 & 3395 & 318 & 168 & 135 & 3395 \\   5 & 277 & 139 & 124 & 3008 & 277 & 139 & 124 & 3008 & 277 & 139 & 124 & 3008 \\    \hline
\end{tabular}\end{center}
\label{tbl:summary}\end{table}
...
\green{A lot more needs to be done here!!!}
