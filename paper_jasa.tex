...
The classical test in this scenario uses $T= \hat{\beta_k}/
se(\hat{\beta_k})$ and rejects the null hypothesis if $T$ is extreme
on the scale of a $t$ distribution with $n-p$ degrees of freedom. This
is the uniformly most powerful (UMP) test \green{reference needed
  here} and it forms the benchmark upon which we evaluate the visual
test. To calculate what we might expect for the power of the visual
test, under perfect conditions, first assume that the observer is able
to pick the plot with the smallest $p$-value from a lineup plot.  This
leads to the decision to reject $H_0$ when $p_{B} < p_0$, where $p_{B}$ is calculated according to details given in Lemma \ref{lemma}. Thus 
the expected power of the visual test in this scenario is
...
\begin{figure}[hbtp]
   \centering
       ...
       \caption{Expected power of visual test  and the power of UMP test for sample size $n =100$, $\sigma = 12$ and $m$=20. \green{Produce these curves for $m=12$, $m=20$, $m=30$.}}
...
\green{Discuss the UMP test \blue{(discussed right after equation \ref{power_exp}) }, and give the reference for it here \blue { I am looking for a good reference. Do you have any suggestion?}.}

\subsection{Continuous covariate} 

This experiment is very similar to the previous one, except that there is a single continuous covariate and no second covariate (Equation \ref{multi} with $p=1$), following the test in Table \ref{tbl:stat_multiple}, case 2. Data is simulated with two sample sizes ($n=100, 300$), two standard deviations of the error ($\sigma=5, 12$), a variety of slopes ($\beta$), as given in Table  \ref{tbl:experiment_params}. We arbitrarily fix $\beta_0 = 6$ and values for $X_1$ are simulated from $N(0,1)$. For each combination of parameters, three different actual data sets are produced, yielding a total of 65 lineups. \green{ I don't understand the next two sentences.} For some parameter combinations we have more than three replications \blue{(since we run an extension of this experiment with more lineups for some parameter combinations}. Unlike what we did in section \ref{sec:category} we did not use any blocking structure to simulate actual data and null plots \blue{(we can ignore this now as I see that blocking discussion in section \ref{sec:category} is removed.) }.
...
An example lineup from this experiment is shown in Figure \ref{fig:test_continuous}.  This lineup is generated using sample size ($n$)=100, slope($\beta$)=1.25 and and error SD($\sigma$)=5. The actual data plot location is 11. \green{Check this} \blue{(Yes, it is 11 !)} This lineup, we think, is a difficult one. To select 10 lineups for a subject, each combination of sample size ($n$) and error SD ($\sigma$) is given a difficulty value based on the slope ($\beta$) parameters. For the smallest slopes the difficulty is 4 (hardest) and for the largest slopes the difficulty is 0 (easiest). For one combination of sample size and standard deviation, the subject sees a randomly selected lineup of each difficulty level, all the slopes, giving 5 lineups. Another four lineups are chosen from a second choice of sample size and standard deviation, with difficulty levels 0-3, and the last lineup shown was a randomly selected lineup with difficulty 0. The order that the lineups are shown to participants is randomized.\blue{The order is as described not randomized after. The randomization is done picking first combination of sample size and sd as well as picking lineups for each combination of parameter} \green{Is this a correct description of the procedure?} Each subject saw a substantial number of difficult lineups. The intention was noble, to get a better handle on the power of the lineups for small slopes, but practically this made is very tiring for the participants. It would be better to provide the subjects with a larger selection of easier lineups and just a few difficult ones. \green{perhaps should go into so much detail about this, or keep this for the conclusions section.}  
...
\green{Discuss the UMP test, and give the reference for it here.}
...
Participants for all the experiments were recruited through \cite{turk} Amazon's Mechanical Turk. Summary of the data we obtained for all three experiments are shown in table \ref{tbl:summary}. Participants are asked to select the plot they think best match the question asked, give a reason for their choice, and determine a level of  confidence for their decision. Gender, age, education and geographic location of each participant are also collected. In this process of collecting data some participants did not provide demographic information as we see in table \ref{tbl:summary} that male and female participants do not add up to total participants. It is not clear why they did not provide the information. Now the concern arises whether we should keep their responses in the study. It is also possible that participants were not serious enough to provide feedback. For each of the experiments we had a test plot (easy plot) for which they should have a correct evaluation without much effort.  This could be a criteria to determine their attentiveness in the study. For this we consider five different data cleaning criteria as shown below;
...
\item include people with 50\% correct on all very easy (could be evaluated correctly if the participant is not ignorant) lineups (as suggested before)
\end{enumerate}
...
\begin{figure}[hbtp]
   \centering
       ...
       \caption{The overall power estimated from model \ref{mixed} are shown for various data sets obtained by applying 5 screening criteria.  The corresponding power curve for Uniformly Most powerful (UMP) test is shown in black solid lines for comparison.}
       \label{fig:power_screening}
\end{figure}

\begin{figure}[hbtp]
   \centering
       \scalebox{0.60}{\includegraphics{power_screening_subject.pdf}}
       \caption{The subject wise power estimated from model \ref{mixed} are shown for various data sets obtained by applying 5 screening criteria.  The corresponding power curve for Uniformly Most powerful (UMP) test is shown in pink lines for comparison.}
...
\begin{figure*}[hbtp]
   \centering
       ...
       \caption{Observed power of visual test from equation \ref{power_est} with pointwise 95\% confidence limits and the power of UMP test for sample size $n= 100,300$ and $\sigma = 12,5$ as per experiment 1.}
...
\begin{figure*}[hbtp]
   \centering
       ...
       \caption{Observed power of visual test from equation \ref{power_est} with pointwise 95\% confidence limits and the power of UMP test for sample size $n= 100$ and $\sigma = 5$ as per experiment 3. }
