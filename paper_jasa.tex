...
\red{Here is what my suggestion for notation \\  Use $B$ for logistic model and $\beta$ for linear model \\ Use $X$ to indicate logistic model covariate matrix and $Y$ to indicate binomial random variable since it is a kind of response. \\ Use $\pi_i$ instead of $p_i$ since $p_i$ is used for p-value. }
