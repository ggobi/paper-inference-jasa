...
\begin{figure}[hbtp]
   \centering
       ...
       \caption{Observed power shown by loess smoother with simultaneous bootstrap confidence band and the power of conventional test. }
       \label{fig:power_screening}
\end{figure}
...
Figure \ref{fig:pval_plot_signal} shows the plot signal strengths in relation with classical test $p$-values. 
...
\hh{For $p$-values below $1/(m-1)$ visual $p$-values are very similar to actual $p$-values, the correlation for those values is: XXX, XXX, and XXX based on $n_1 = YYY, n_2 = YYY$, and $n_3=YYY$ lineups, respectively. Mahbub, could you please compute correlation coefficients for $p$ values below $1/(m-1)$ and fill in the numbers. } 
...
Another difference between classical tests and visual inference is the potential influence that null plots have on an observer's choice of plot. If one or more null plots represent data with -- by chance -- a smaller $p$-value than the data plot, there's a strong possibility that an observer will pick the null plot instead of the data plot, independently of the actual value of the $p$-value. 
