...
\red{HH: I don't think power should be defined in terms of $\theta$ Usually power is considered a property of the test.  In our case that's the specifics of the plot used for the lineup. We might need an additional definition for that - maybe we can phrase that within the framework of the grammar of graphics? - that's what I've tried above}

For a lineup of $m$ plots the power of \blue{a visual test}
%$\theta$
 is defined as 
    \begin{equation*}
      \text{Power}(T_\theta)= 
    ...
    \end{equation*}
\end{dfn}
\red{this is a non-standard definition of power - usually power is just the prob to reject under $H_1$. could we back the non-standard definition up with some literature?}
...
\red{second step: the p-values under $H_0$ are uniform.}
