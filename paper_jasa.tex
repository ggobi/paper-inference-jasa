...
Now consider estimating the power of the visual test. For the regression slope parameter $\beta$ suppose the associated $p$-value be $p_B$ and its distribution function be  $F_{p_B}(.)$ . As noted by \cite{Ruppert:2007} $F_{p_B}(.)$ is a right skewed distribution under $H_1$ and uniform under $H_0$.  In a lineup plot we simulate $m-1$ residual data sets from null model where each of these $m-1$ null data sets produces a corresponding $p$-value $p_{0,i}$ and $p_{0,i} \sim \text{Uniform}(0,1)$ for $i = 1, ..., m-1$. Suppose  $p_0=\min_i(p_{0,i})$. Thus $p_0 \sim \text{Beta}(1,m-1)$. 
