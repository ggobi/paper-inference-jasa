...
{\bf Screening scenarios:} Amazon Mechanical Turk data often suffers from the problem that some participants are trying to 'game' the system -- in our situation, these are participants who did not put in a best effort to identify the data plot, but just randomly picked a plot to maximize their 'winnings'.  In order to avoid using those results, we are comparing six different screening methods for excluded participants from the data analysis. 
...
Table \ref{tbl:summary} shows  the summary results after applying different screening on the data. For our analysis we picked criteria six (6) and present results based on this in the following sections.
