...
The $(m-1)$ null plots show samples drawn from the sampling distribution under $H_0$. %of the test statistic assuming that the null hypothesis is true. 
...
\blue{This gives us a tool to evaluate different types of graphics with respect to their efficiency:} for two different visual test statistics of the same observed data, the one  is \blue{more efficient}, in which a specific pattern is more easily distinguishable visually. This should be reflected in the power of the test. We can assess power therefore both empirically based on experimental data and through theoretical considerations. Next, we will develop power theoretically and then %It is feasible to assess power theoretically and 
