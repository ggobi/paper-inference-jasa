...
When $K$ individuals evaluate a lineup plot independently, the number of successful evaluations, $U \sim \text{Binom} (K,1/m)$  under $H_0$. Thus we define $p$-value for visual test $T_{\theta}$ as in definition \ref{dfn:pval}.
\begin{dfn} \label{dfn:pval}
The $p$-value of A visual test $T_{\theta}$ is at most $Pr(U \ge u)= \sum_{k \ge u}^K {{K \choose k} (\frac{1}{m})^k(1-\frac 1m)^{(K-k)}}$ where $u$ is the observed number of successful evaluations.
\end{dfn}
...
\red{MM: This $t$ comes from the idea that a classical $T-test$ is available for which we are describing the visual test. Generalizing it is a little bit technical since for all visual test, we may not have a classical $T-test$. Thus the concept of $F(.)$ is little confusing too. I don't know how deal with this situation here.}\\
...
\red{MM: $p_D$ and $p_B$ are same, right?}
