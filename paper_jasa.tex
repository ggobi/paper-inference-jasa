...
A \textbf{visual test (statistic)} $T(y)$ is as a plot of the sample, and $T_0$ are plots of null data. \red{MM: I was thinking, visual test statistic $T(.)$ is a function that maps (using the grammar of graphics) data into a statistical plot. For observed data ($y$) we have observed test statistic (plot) $T(y)$ and for null data $y_0$ we have $T(y_0)$ which could be denoted by $T_0$.}

\end{dfn}

Ideally, the visual test statistic is defined using the grammar of graphics\citep{wilkinson:1999,hadley:2009}, consisting of type and specification of aesthetics necessary for complete reproducibility. The visual test statistic is compared with values $T_0$ using a lineup, which is defined as:

\begin{dfn}\label{dfn:lplot}
A \textbf{lineup}  is a layout of $m$ visual statistics, consisting of 
\begin{itemize}\itemsep-3pt
\item $m-1$ statistics, $T_0$ simulated from the model specified by $H_0$  (null plots) and 
\item the test statistic, $T(y)$ produced by plotting the observed data, possibly arising from $H_1$.
\end{itemize}
\end{dfn}

\noindent The $(m-1)$ null plots can be considered to be samples drawn from the sampling distribution of the test statistic assuming that the null hypothesis is true.
If $H_1$ is true, we expect this to be reflected as a feature in the test statistic, i.e. the plot of the data, that makes it visually distinguishable from the null plots. {\em If the test statistic cannot be identified} in the lineup the conclusion is to {\em not reject the null hypothesis.}  

A careful visual inspection of the lineup by an independent observer follows, and the observer is asked to point out the plot most different from the lineup.  Since the lineup plot consists of $m$ plots, the probability of choosing any one of them is $1/m$. Thus we have type-I error probability of $1/m$.
