...
\newcommand{\red}[1]{{\color{red} #1}} %HH
...
\[
\text{logit } p_{\ell i} = X_{\ell i} B +  Z_{\ell i} \tau_i,  
\]
where $X_{\ell i}$ is a vector of fixed effects with parameter vector $B$ corresponding to lineup $\ell$ and $\tau \sim MVN(0, \sigma_{\tau} I)$  is a vector of random variables $i = 1, ..., K$.

\red{This model is elaborately described in equation \ref{mixed}.}
...
Suppose each of $K$ independent observers gives evaluations on multiple lineup plots and responses are associated with binary random variable $Y_{\ell i}$. Let $Y_{\ell i} = 1$, if subject $i$ correctly identifies the test statistic on lineup $\ell$, and 0 otherwise.
We model $\pi_{\ell i} = E(Y_{\ell i})$ as  a mixed effects logistic regression

\begin{equation} 
g(\pi_{\ell i}) = X_{\ell i}B  + Z_{\ell i} \tau_i 
\label{mixed} 
\end{equation}
%
where $\tau_i$ is random effects coefficient vector of length $q$ for subject $i$,  $\tau_i  \sim  MVN(0,\Sigma)$ with variance covariance matrix $\Sigma$, $Z_{\ell i}$ is the $\ell$th row vector of random effects covariates for subject $i$, $B$ is a vector of coefficient of length $p$, the number of fixed effect covariates being used, $X_{\ell i}$ is the $\ell$th row vector of the fixed effects covariates for subject $i$ and logit link function $g(\pi)=\log(\pi) - \log(1-\pi); 0 \le \pi \le 1$. The covariates could be demographic information of people such as age, gender, education level etc. as well as sample size ($n$), regression parameter ($\beta$) and variance ($\sigma^2$) of model \ref{multi}. 
