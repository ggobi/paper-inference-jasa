...
Using the inverse {\it logit} link function, $g^{-1}(.)$, from  Equation \ref{eqn:mixed} we obtain the the estimate of subject and lineup specific probability of successful evaluation as 
...
Another difference between conventional tests and visual inference is that the observer's choice of plot may be influenced by the $m-1$ null plots in the lineup. We have already seen that observers tend to pick the plot with the lowest $p$-value. If one or more null plots represent data having -- by chance -- a smaller $p$-value than the actual data plot, there's a strong possibility that an observer will pick this null plot instead of the actual data plot. 

Figure \ref{fig:pval_difference} investigates this for the data collected in experiments 1 and 2. The horizontal axis shows the difference between the smalles $p$-value of the null plots, and that of the actual data plot. Large values indicate that the actual data plot has a much smaller $p$-value than any of the null plots. The vertical axis shows percent subjects choosing the actual data plot. \green{Is this really pct correct, or 1-pct correct? When there is a big diff in p-value almost no-one gets it right! Maybe it is just a scale issue?} We see that when there is a big difference between the $p$-values observers choose the actual data plot more often.
...
This problem is further investigated in the paper \cite{waldo}. Here numerical measures of the similarity between plots are proposed to provide quality metrics for lineups.
