...
The model in Equation \ref{eqn:mixed} provides the subject and lineup specific probability of successful evaluation as 
\begin{equation} \label{eqn:mixed_power}
\hat p_{\ell i} = \frac{exp(\hat g_{\ell i})}{1+ exp(\hat g_{\ell i})}
\end{equation}
where $\hat g_{\ell i} = W_{\ell i} \hat {\delta} +  Z_{\ell i}  \hat {\tau}_{\ell i}$.
...
\begin{table}[hbtp]
\caption{Parameter estimates of model \ref{eqn:mixed}.}
\begin{center}
\begin{tabular}{ccc}
  \hline
