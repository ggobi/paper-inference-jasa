...
{\bf Plotting visual statistic:} For a given data set we fit the null model and obtain the residuals.  Using these residuals the visual statistics for each of the experiments were generated as described in table \ref{tbl:stat_multiple}. Actual data plot and null plots in a lineup are obtained from actual data set and null data set respectively. The procedure for obtaining actual data sets are described in the following sections.



\subsection{Discrete covariate}\label{sec:category}

The experiment is designed to study the ability of human observers to detect the effect of a single variable $X_2$ (corresponding to parameter $\beta_2$) in a two variable ($p=2$) regression model (Equation \ref{multi}). Data is simulated a range of values of $\beta_2~ (=0, 1, 3, 5, 7, 8, 10, 16)$, two different sample sizes ($n=100, 300$) and two standard deviations of the error ($\sigma=5, 12$). The range of $\beta_2$ values was chosen so that estimates of the power would produce reasonably continuous power curves, comparable to that calculated for the theoretical UMP test. Values were fixed for other regression parameters, $\beta_0 = 5$,  $\beta_1=15$, and the values for $X_1$ were randomly generated from a Poisson(30) distribution. \green{Explain why this was used.} \red{There was not any real reason for this. It was just arbitrarily chosen to generate some data. When I first started to think of applying visual inference in regression setting, I was thinking of a dataset (I found in a book) of weight of babies by their gestational age (in week) where author wanted to test the difference of mean weight for gender.   Thus for our experiment I generated values similar to a variable like age. the discrete variable was thought similar to gender.} Data sets were generated with parameter values shown in Table \ref{tbl:experiment_params}. Three replicates at each level were generated. These produced 60 different ``actual data sets''. 
