...
K <- 10
qplot(m, power, geom="line", group=pb, data=powerdf, colour=pb) + geom_line(aes(m, 1/m), colour="grey30", lty=2) + geom_text(aes(label=sprintf("%.2f",pb)), data=subset(powerdf, m==5), hjust=1.1) + xlim(c(0,50)) + scale_colour_gradient(name=expression(p[B])) + geom_ribbon(aes(x=m, ymin=1/m-1.96*sqrt(1/m*(m-1)/m)/K, ymax=1/m+1.96*sqrt(1/m*(m-1)/m)/K), fill="grey80", lty=0, alpha=0.1)
