...
included <- unique(turker$id[with(turker, correct/n >= 0.1)])
...
base <- lmer(response~ offset(offset) - 1 + effect + (effect|id) , data=dat, family=binomial())
anova(m0, base)
q11 <- qplot(ranef(base)[[1]][,1], binwidth=0.2)
q12 <- qplot(ranef(base)[[2]][,1], binwidth=0.2)
summary(base)


base.1 <- lmer(response~ effect + (effect|id) , data=dat, family=binomial())


m1 <- lmer(response~ effect + (1|id), data=dat, family=binomial())
anova(base, m1)

m1a <- lmer(response~ effect + gender + (1|id), data=dat, family=binomial())
m1b <- lmer(response~ effect + academic_study + (1|id), data=dat, family=binomial())
m1c <- lmer(response~ effect + age + (1|id), data=dat, family=binomial())

m2a <- lmer(response~ effect + mgender + (1|id), data=dat, family=binomial())

anova(m0, base,  m2a, m1a)
anova(base, m1,  m2a, m1b)
anova(base, m2a, m1c)

dat$alpha <- 0.05
m3c <- lmer(response~ offset(alpha)+effect -1 + age + (effect|id), data=dat, family=binomial())
anova(base, m2a, m1c, m3c)
