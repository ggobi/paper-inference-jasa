<<read-data, echo=FALSE, resuls=hide>>=

suppressMessages(library(ggplot2))
setwd("/Users/heike/papers/paper-inference-jasa/R_codes/")
...
# where are our reference charts?
# we should not draw inference on charts that we use for study inclusion

<<select, dependson=read-data, echo=FALSE, resuls=hide>>=
raw.dat$response <- raw.dat$response_no == raw.dat$plot_location

raw.dat$effect <- with(raw.dat, beta/sigma*sqrt(sample_size))

raw.dat$gender <- factor(raw.dat$gender, exclude=NULL)
levels(raw.dat$gender) <- c("male", "female", "unknown")
raw.dat$mage <- is.na(raw.dat$age)
raw.dat$meduc <- is.na(raw.dat$academic_study)

raw.dat$academic_study <- addNA(factor(raw.dat$academic_study))
raw.dat$age <- addNA(factor(raw.dat$age))

raw.dat$mgender <- raw.dat$gender=="unknown"

#dp <- ddply(subset(raw.dat, p_value < 0.0002),.(id),summarise,
#   percent_correct = mean(response)*100
#  )

#included_id <- dp$id[ dp$percent_correct > 0]
#dat <- subset(raw.dat, id %in% included_id)
dat <- raw.dat

suppressMessages(require(plyr))
turker <- ddply(dat,.(id), summarise,
	n = length(id),
	correct = sum(response[p_value < 0.0002]),
  gotchance = sum(p_value < 0.0002),  
  pvalue = mean(p_value),  
  pcorrect = sum(response)/length(id),
  gender=gender[1]
)

## if we include all correct percentages above 0.2 we get rid of any differences due to gender, age or schooling

## Mahbub
# included <- unique(turker$id[with(turker, correct/gotchance > 0.5)])

## if we exclude everybody with just a single lineup, we have the difference introduced by missing values show up massively
#included <- unique(turker$id[turker$n > 1])

## if we exclude everybody with just a single lineup, we have the difference introduced by missing values show up massively
#included <- unique(turker$id[turker$gender != "unknown"])

dat <- subset(raw.dat, id %in% included)


lineups <- ddply(dat,.(pic_id), summarise,
	n = length(id),
...
<<fig-basic, dependson=select, echo=FALSE>>=
qplot(effect, p_value, data=lineups, colour=correct/n)
qplot(effect, correct/n, data=lineups, colour=p_value) + geom_smooth(method="loess")
@

<<model0, dependson=select>>=
suppressMessages(library(lme4))
dat$setup <- with(dat, sprintf("%d:%d:%d", beta, sigma, sample_size))

m0 <- lmer(response~ 1 + (1|id) + (1|setup), data=dat, family=binomial())
q01 <- qplot(ranef(m0)[[1]][,1], binwidth=0.2)
q02 <- qplot(ranef(m0)[[2]][,1], binwidth=0.2)
summary(m0)
summary(ranef(m0)[[1]][,1])
summary(ranef(m0)[[2]][,1])

dat$offset <- log(0.05/.95)
base <- lmer(response~ offset(offset) - 1 + effect + (effect-1|id) , data=dat, family=binomial())

qqnorm(ranef(base)[[1]][,1])
q11 <- qplot(ranef(base)[[1]][,1], geom="histogram", binwidth=0.002)
q11

base.2 <- lmer(response~ offset(offset) - 1 + effect + (effect-1|id) , data=dat, family=binomial(link="probit"))
base.3 <- lmer(response~ offset(offset) - 1 + effect + (effect-1|id) , data=dat, family=binomial(link="cloglog"))
# 
anova(base.2, base.3, base)



m1a <- lmer(response~  offset(offset) - 1 + effect:mgender + (effect-1|id), data=dat, family=binomial())
m1b <- lmer(response~ offset(offset) - 1 + effect:academic_study + (effect-1|id), data=dat, family=binomial())
m1c <- lmer(response~ offset(offset) - 1 + effect:age + (effect-1|id), data=dat, family=binomial())

anova(base, m1a)
anova(base, m1b)
anova(base, m1c)

qplot(effect, fitted(m1a), data=dat, colour=factor(academic_study))
# there's something in model m1c that's not explained by missingness alone - but it might be an artefact - there's nothing immediately visible in the estimates
@

<<plots, dependson=model0, echo=FALSE, results='hide', out.width='\\text.width'>>=
qplot(ranef(m3c)[[1]][,1], binwidth=0.1) + xlab("Individuals' Skill")

qqnorm(ranef(m1c)[[1]][,1])
abline(a=0,b=1)


qplot(effect, fitted(base), data=dat, colour=difficulty,  group=factor(sample_size)) + geom_smooth() 

qplot(effect, fitted(m1c), data=dat, colour=factor(sample_size))
qplot(group=effect, x=resid(m1c), geom="histogram", data=dat, facets=~effect, binwidth=0.2)
@

levels(dat$age) <- c("below 18", "18 - 25", "26 - 30", "31 - 35", "36 - 40", "41 - 45", "46- 50", "51 - 55", "56 - 60", "above 60")
...
<<predict, dependson=model0>>=
predict.lme <- function(model, newdata, skills=0) {
  design <- ldply(newdata$age, function(x) levels(model@frame$age) %in% x)
  xnew <- cbind(effect=newdata$effect, design)
  preds <- drop(as.matrix(xnew) %*% fixef(model)) + skills
  exp(preds)/(exp(preds)+1)
}

newdata <- data.frame(expand.grid(effect=seq(0,17, by=0.1), age=c(0:10, NA)))
newdata$m1c <- predict.lme(m3c, newdata=newdata)

newdata_all <- data.frame(expand.grid(effect=seq(0,17, by=0.1), age=c(0:10, NA), skills=ranef(m1c)[[1]][,1]))
newdata_all$m1c <- predict.lme(m1c, newdata=newdata_all, skills=ranef(m1c)[[1]][,1])


qplot(effect, m1c, data=newdata, colour=factor(age))
qplot(effect, m1c, data=newdata_all, colour=factor(age))
@
