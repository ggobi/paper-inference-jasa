...
The materials in this document supplement the information presented in the manuscript ``Validation of Visual Statistical Inference, Applied to Linear Models".  Section \ref{sec:lineup_selection} describes how the lineups are presented to the subjects for evaluation, section \ref{sec:proof_lemma} presents the proof of the Lemma \ref{p-lemma} in the manuscript. The detailed discussion on how much the choice of null plots affect is included in Section \ref{sec:null_choice} which supplement the information in Section \ref{p-sec:null_choice} of the manuscript. Section \ref{sec:typeIII_error} includes the detailed discussion about the Type-III error supplementing Section  \ref{p-sec:typeIII_error}. The data cleaning process and how that may affect the result are discussed in Section \ref{sec:data_cleaning} which supplements Section \ref{p-sec:data_cleaning} of the manuscript.

\section{Selection of Lineups for each subject} \label{sec:lineup_selection}
Table \ref{tbl:dist_lineup1} shows how the lineups are presented to the subjects for evaluation for the simulation experiment with categorical covariate described in Section \ref{p-sec:category} of the manuscript . The lineups are divided into four groups such as easy, medium, hard and mixed based on the parameter combinations shown in the table. The number of lineups randomly chosen for each difficulty level is shown in the table as well. Note that every subject saw at least one easy lineup in the pool of 10 lineups shown. Some subjects saw more than one easy lineup that comes from the mixed group. The number of total evaluations required for each lineup is also shown in the table and a total of 300 subjects would provide a total of 3000 evaluations that should roughly satisfy that plan.  
...
Figure \ref{p-fig:power_loess_effect} in the manuscript indicates that Type III error might be occurring in experiment 3: correct identification of the actual data plot is not positively associated with effect size. Teasing this out of the results is possible by looking at the reasons observers gave for their choices. Observers were provided with four possible reasons to use for their choice:
...
Following six approaches have been examined while cleaning the data.  Table \ref{tbl:summary} displays number of subjects and their total evaluations in each screening criteria. The criteria 1 includes all the participants hence showing number of total subjects participated in the survey for all the experiments. For our paper we used criteria six to clean the data and the results presented in the paper are based on the cleaned data.
